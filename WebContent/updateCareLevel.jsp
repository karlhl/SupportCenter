...
<body>
	...
	<h1 class="color">护理等级更新界面</h1>
	<!--表单标签-->
	<div style="color: #FF0000; font-size: 36px; font-weight: bold;"
		id="errorMessage">${requestScope.msg}</div>
	<form name="myform" action="/SupportCenter/LevelUpdateController"
		method="get">
		<!--表单元素-->
		护理等级ID:<input type="text" name="id" id="id"  value="${requestScope.CareLevel[0].ID}" /><br /> 
		护理等级名称:<input
			type="text" name="name" id="name"  value="${requestScope.CareLevel[0].LEVEL_NAME}"  /><br /> 
		护理等级状态:<select name="status" id="status"  value="${requestScope.CareLevel[0].LEVEL_STATUS}" >
